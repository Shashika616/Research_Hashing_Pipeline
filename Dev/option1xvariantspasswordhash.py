# MODIFIED VERSION
# Plot-only and Plot+Argon2 now use: Option 1 - map each byte of combined_input individually
...
def compare_methods(password: str):
    ...
    # Plot-only (Option 1: map each byte in combined_input individually)
    ...
    # values = [apply_plot_function(*map_char_to_function_with_x(chr(b), salt1, salt2)) for b in combined_input]
    # binary_data = struct.pack(f'{len(values)}f', *values)
    # print(f"[DEBUG] Binary Data (hex): {binary_data.hex()}")
    # print(f"[DEBUG] Binary Data (raw float values): {values}\n")
    ...
    for b in combined_input:
        ...
        result = apply_plot_function(plot_type, p1, p2, x)
        values.append(result)
        x_values.append(x)

    binary_data = struct.pack(f'{len(values)}f', *values)

    print(f"\n[DEBUG] x values used (float): {x_values}")
