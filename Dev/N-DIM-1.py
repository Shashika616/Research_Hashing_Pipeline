...
from sympy import symbols, expand, sstr
...
def normalize_multi_dim_x_values_to_custom_range(xs_list: list[list[float]], x_min: float, x_max: float, method: str = "linear") -> list[list[float]]:
    """
    Normalize per-character vectors into [x_min, x_max] using different methods:
      - linear: standard 0..1 linear scaling
      - log: log(1 + |x|) scaling
      - clipped: clip extreme values to 1st/99th percentile before linear scaling
      - tanh: tanh-based non-linear scaling
...
def apply_plot_function(plot_type: int, p1: float, p2: float, x_list: List[float]) -> Tuple[float, str]:
    """
    Generates a user+character-specific numeric function based on plot_type
    and evaluates it for x_list.
    ...
    Computes numeric value and string representation of a user+character-specific function.

    Returns:
        y_value (float)   : numeric evaluation
        func_repr (str)   : human-readable function expression
    """
    n = len(x_list)
    expr_val = 0.0
    expr_str_parts = []
...
def plot_results(passwords, all_times, all_entropies, all_output_lens, methods):
    x = np.arange(len(passwords))
    width = 0.12
    ...
    axs[0].set_xticks(x); axs[0].set_xticklabels(passwords); axs[0].set_ylabel("Hashing Time (s)"); axs[0].set_title("Hashing Time Comparison"); axs[0].legend(); axs[0].grid(True)
    axs[1].set_xticks(x); axs[1].set_xticklabels(passwords); axs[1].set_ylabel("Entropy (bits)"); axs[1].set_title("Entropy Comparison"); axs[1].legend(); axs[1].grid(True)
    axs[2].set_xticks(x); axs[2].set_xticklabels(passwords); axs[2].set_ylabel("Output Length (bytes)"); axs[2].set_title("Output Length Comparison"); axs[2].legend(); axs[2].grid(True)
    plt.tight_layout()
    plt.show()

def plot_intermediate_x_values(x_values_multi):
    n_samples = len(x_values_multi)
    n_dims = max(len(vec) for vec in x_values_multi)
    fig, axes = plt.subplots(n_dims + 1, 1, figsize=(10, 3*(n_dims+1)), constrained_layout=True)
    for dim in range(n_dims):
        y_dim = [vec[dim] if dim < len(vec) else 0 for vec in x_values_multi]
        axes[dim].plot(range(n_samples), y_dim, marker='o', linestyle='-', label=f'Dim {dim+1}')
        axes[dim].set_title(f'Dimension {dim+1}'); axes[dim].set_xlabel('Character index'); axes[dim].set_ylabel('x value'); axes[dim].legend(); axes[dim].grid(True)
    for dim in range(n_dims):
        y_dim = [vec[dim] if dim < len(vec) else 0 for vec in x_values_multi]
        axes[-1].plot(range(n_samples), y_dim, marker='o', linestyle='-', label=f'Dim {dim+1}')
    axes[-1].set_title('All Dimensions Combined'); axes[-1].set_xlabel('Character index'); axes[-1].set_ylabel('x value'); axes[-1].legend(); axes[-1].grid(True)
    plt.show()

def plot_heatmap_x_values(x_values_multi):
    data = np.array([vec + [0]*(max(len(v) for v in x_values_multi)-len(vec)) for vec in x_values_multi])
    plt.figure(figsize=(12, 6))
    plt.imshow(data.T, cmap='viridis', aspect='auto')
    plt.colorbar(label='x value')
    plt.xlabel('Character Index')
    plt.ylabel('Dimension')
    plt.title('Heatmap of Multi-Dimensional x-values per Character')
    plt.show()    
...
# --- Plot intermediate x-values per character for multi-dimensional transformations ---
_, _, x_values_multi = iterative_plot_transform(pwd, b'\x00'*8, b'\x00'*8, iterations=2)  # or retrieve from compare_methods
if x_values_multi:
    n_samples = len(x_values_multi)
    n_dims = max(len(vec) for vec in x_values_multi)
    fig, axes = plt.subplots(n_dims + 1, 1, figsize=(10, 3*(n_dims+1)), constrained_layout=True)
    ...
    for dim in range(n_dims):
        ...
        axes[dim].set_xlabel('Character index')
        axes[dim].set_ylabel('x value')
        axes[dim].legend()
        axes[dim].grid(True)

    # Plot all dimensions combined
    for dim in range(n_dims):
        y_dim = [vec[dim] if dim < len(vec) else 0 for vec in x_values_multi]
        axes[-1].plot(range(n_samples), y_dim, marker='o', linestyle='-', label=f'Dim {dim+1}')
    axes[-1].set_title('All Dimensions Combined')
    ...
    plt.show()
    # plot_results(passwords, all_times, all_entropies, all_output_lens, methods)
    # plot_intermediate_x_values(x_values_multi)
    plot_heatmap_x_values(x_values_multi)
