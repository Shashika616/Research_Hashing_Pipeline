...
# ----------------------------
# Iterative plot transform with experiments (Δy for all chars)
# ----------------------------
...
def iterative_plot_transform(password: str, salt1: bytes, salt2: bytes, iterations: int = 2):
    ...
    for it in range(1, iterations + 1):
        ...
        # Δy experiment for all characters
        delta = 1e-7
        delta_ys = []
        ...
        for b in combined_input:
            ...
            delta_ys.append(abs(y2 - y1))  # compute Δy per character

            # Compute intermediate value for transformation
            values.append(y1)

        # Aggregate Δy (max or mean)
        delta_y = max(delta_ys)  # could also use sum(delta_ys)/len(delta_ys) for mean

        # Pack intermediate floats
        binary_data = struct.pack(f'{len(values)}f', *values)
        all_intermediates.append(binary_data)

        # FP32/FP64 collapse, preimage multiplicity, entropy etc. as before
        tracemalloc.start()
        ...
        # Preimage multiplicity (simple test with 1000 x values around first char)
        ...
        bucket = {}
        for val in xs:
            fp32_val = struct.pack(">f", np.float32(apply_plot_function(plot_type, p1, p2, val)))
            bucket[fp32_val] = bucket.get(fp32_val, 0) + 1
...
if __name__ == "__main__":
    ...
    for pwd in passwords:
        salt1 = os.urandom(8)
        salt2 = os.urandom(8)
        final_binary, intermediates, metrics = iterative_plot_transform(pwd, salt1, salt2, iterations=2)
        ...
        for i, (data, m) in enumerate(zip(intermediates, metrics),1):
            ...
            print(f"Entropy={m['entropy']}, Length={m['length']}, Memory={m['memory_kb']} KB, "
