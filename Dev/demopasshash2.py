...
def map_chunk_to_function(chunk: bytes) -> Tuple[int, float, float]:
    # Pad chunk if less than 8 bytes
    if len(chunk) < 8:
        chunk += b'\x00' * (8 - len(chunk))
    
    plot_type = chunk[0] % 6
    p1 = struct.unpack('f', chunk[1:5])[0]  # bytes 1-4
    p2 = struct.unpack('f', chunk[5:9])[0]  # bytes 5-8
    
    return plot_type, p1, p2

def map_digest_chunks_to_functions(digest: bytes, chunk_size: int = 8):
    values = []
    for i in range(0, len(digest), chunk_size):
        chunk = digest[i:i+chunk_size]
        plot_type, p1, p2 = map_chunk_to_function(chunk)
        val = apply_plot_function(plot_type, p1, p2)
        values.append(val)
    return values
...
def compare_methods(password: str):
    ...
    # Plot-only with Option 2: Use combined input digest split into chunks
    combined_input = password.encode() + salt1 + salt2
    digest = hashlib.sha256(combined_input).digest()
    values = map_digest_chunks_to_functions(digest, chunk_size=3)
    binary_data = struct.pack(f'{len(values)}f', *values)
    tracemalloc.start()
    start = time.time()
    plot_hash = hashlib.sha256(binary_data).digest()
    ...
    # Plot+Argon2 with same new plot hash input
    hasher = PasswordHasher(time_cost=2, memory_cost=102400, parallelism=8)
    tracemalloc.start()
    start = time.time()
    plot_argon_hash = hasher.hash(binary_data)
