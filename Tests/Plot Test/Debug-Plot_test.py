...
open("debug_plot_transformations.txt", "w").close()

LOG_FILE = "debug_plot_transformations.txt"

def log(msg):
    with open("debug_plot_transformations.txt", "a", encoding="utf-8") as f:  # <- add encoding
        f.write(sstr(msg) + "\n")
...
def dbg(msg, /, *args):
    if DEBUG:
        if isinstance(msg, str):
            ...
            formatted = str(msg)  # <-- convert non-strings (like numpy arrays) to string
        print(formatted)
        log(formatted)

def dbg_bytes(label: str, b: bytes):
    # Show first 64 bytes or full length if smaller
    snippet = b[:64]
    hex_str = " ".join(f"{x:02x}" for x in snippet)
    print(f"{label} (len={len(b)} bytes, showing first {len(snippet)} bytes): {hex_str}")
    dbg(f"{label} (len={len(b)} bytes, showing first {len(snippet)} bytes): {hex_str}")
...
def map_char_to_function_with_x(char: str, salt1: bytes, salt2: bytes, char_index: int, plot_count: int, max_dims: int = 8) -> Tuple[int, float, float, int, List[float]]:
    ...
    plot_type = hmac_type[0] % plot_count
...
def bytes_to_bitarray(b: bytes) -> np.ndarray:
    """Return numpy array of bits (0/1) for a bytes object."""
    # convert to uint8 array then to bits
    arr = np.frombuffer(b, dtype=np.uint8)
    bits = ((arr[:, None] & (1 << np.arange(8)[::-1])) != 0).astype(np.uint8)
    return bits.reshape(-1)

def bit_hamming_distance(b1: bytes, b2: bytes) -> int:
    a = bytes_to_bitarray(b1)
    c = bytes_to_bitarray(b2)
    L = min(a.size, c.size)
    # if different lengths, pad shorter with zeros (should not happen for current_input which is fixed-size)
    if a.size != c.size:
        maxlen = max(a.size, c.size)
        a = np.pad(a, (0, maxlen - a.size), 'constant', constant_values=0)
        c = np.pad(c, (0, maxlen - c.size), 'constant', constant_values=0)
    return int(np.count_nonzero(a != c))

def byte_xor_count(b1: bytes, b2: bytes) -> int:
    la = len(b1)
    lb = len(b2)
    L = min(la, lb)
    cnt = sum(1 for i in range(L) if (b1[i] ^ b2[i]) != 0)
    if la != lb:
        # count remaining bytes as different
        cnt += abs(la - lb)
    return cnt

def test_avalanche_vary_plot_count(password: str, iterations: int, max_plot_count: int, flip_index: int = -1):
    """
    For plot_count in 1..max_plot_count:
      - compute final_input (32 bytes) for password
      - compute final_input for password' which differs by one character (flip_index toggled)
      - compute bitwise normalized Hamming and byte-xor count
    flip_index: index in password to modify (default -1 -> last char)
    """
    # fixed salts so only transformation differences matter
    salt1 = os.urandom(8)
    salt2 = os.urandom(8)

    # build modified password by flipping the character at flip_index
    if len(password) == 0:
        raise ValueError("password must be non-empty")
    idx = flip_index if flip_index >= 0 else len(password) + flip_index
    if idx < 0 or idx >= len(password):
        idx = len(password) - 1

    orig_char = password[idx]
    # produce a deterministic nearby char (rotate codepoint)
    new_char = chr((ord(orig_char) + 1) % 0x110000)  # safe within Unicode range
    modified_password = password[:idx] + new_char + password[idx+1:]

    results = []
    for pcount in range(1, max_plot_count + 1):
        # compute original
        out1, _, _ = iterative_plot_transform(password, salt1, salt2, iterations, plot_count=pcount)
        out2, _, _ = iterative_plot_transform(modified_password, salt1, salt2, iterations, plot_count=pcount)

        # final outputs are bytes (SHA256 digests), confirm length
        assert isinstance(out1, (bytes, bytearray)) and isinstance(out2, (bytes, bytearray))

        bits_diff = bit_hamming_distance(out1, out2)
        total_bits = max(len(out1), len(out2)) * 8
        bit_diff_norm = bits_diff / total_bits

        bxor = byte_xor_count(out1, out2)
        byte_norm = bxor / max(len(out1), len(out2))

        results.append({
            "plot_count": pcount,
            "bits_diff": bits_diff,
            "bit_diff_norm": bit_diff_norm,
            "byte_xor": bxor,
            "byte_xor_norm": byte_norm,
            "out1_hex": out1.hex(),
            "out2_hex": out2.hex()
        })

        print(f"plot_count={pcount:2d}  bit_diff={bits_diff:3d}/{total_bits} ({bit_diff_norm:.3%})  byte_xor={bxor}/{len(out1)} ({byte_norm:.3%})")
        dbg(f"plot_count={pcount:2d}  bit_diff={bits_diff:3d}/{total_bits} ({bit_diff_norm:.3%})  byte_xor={bxor}/{len(out1)} ({byte_norm:.3%})")
...
def iterative_plot_transform(password: str, salt1: bytes, salt2: bytes, iterations: int, plot_count: int):
    ...
    for it in range(1, iterations + 1):
        ...
        for idx, b in enumerate(combined_input):
            ...
            x_raw_values.append(mapping[4]) #Appends x_list
        ...
        for i, (plot_type, p1, p2, n, _) in enumerate(char_maps):
            ...
            # dbg("    FUNCTION idx={}| char='{}' -> {}", i, char, func_str)
            # print("    OUTPUT   idx={}| char='{}' -> {}", i, char, result)
            # dbg("    OUTPUT   idx={}| char='{}' -> {}", i, char, result)
...
def compare_methods(password: str):
    ...
    final_binary, all_intermediates, x_values_multi = iterative_plot_transform(password, salt1, salt2, iterations, plot_count=MAX_PLOTS)
...
CSV_FILE = "plot_avalanche_results.csv"

#   Avalanche test: vary the number of plots

PASSWORD = "password123"       # fixed password for the experiment
SALT1 = b"fixed_salt_1__"      # fixed salts (use deterministic values for reproducibility)
SALT2 = b"fixed_salt_2__"
ITERATIONS = 2                 # use 1 iteration of your pipeline (or other value you want)
MAX_PLOTS = 51                 # test plot_count = 1..MAX_PLOTS (0..50 types)
CSV_OUTPUT = "avalanche_all_pairs.csv"
VERBOSE = True

argon_hasher = PasswordHasher(time_cost=2, memory_cost=102400, parallelism=8)

def bytes_to_bitarray(b: bytes) -> np.ndarray:
    """Convert bytes to bit array (0/1 numpy uint8), big-endian per byte."""
...
def bit_hamming(b1: bytes, b2: bytes) -> int:
    """Count bit differences (pads shorter with zeros)."""
    a = bytes_to_bitarray(b1)
    c = bytes_to_bitarray(b2)
    if a.size < c.size:
        a = np.pad(a, (0, c.size - a.size), 'constant', constant_values=0)
    elif c.size < a.size:
        c = np.pad(c, (0, a.size - c.size), 'constant', constant_values=0)
    return int(np.count_nonzero(a != c))

def bit_hamming_percent(b1: bytes, b2: bytes) -> float:
    bits = max(len(b1), len(b2)) * 8
    if bits == 0:
        return 0.0
    return bit_hamming(b1, b2) / bits * 100.0

def byte_xor_count(b1: bytes, b2: bytes) -> int:
    la = len(b1)
    lb = len(b2)
    L = max(la, lb)
    # pad shorter with zeros for XOR counting
    if la < L:
        b1 = b1 + b'\x00' * (L - la)
    if lb < L:
        b2 = b2 + b'\x00' * (L - lb)
...
def byte_xor_percent(b1: bytes, b2: bytes) -> float:
    L = max(len(b1), len(b2))
    if L == 0:
        return 0.0
    return byte_xor_count(b1, b2) / L * 100.0

def produce_outputs_for_plot_counts(password: str, salt1: bytes, salt2: bytes, iterations: int, max_plots: int):
    """
    For each plot_count in 1..max_plots:
      - run iterative_plot_transform(password, salt1, salt2, iterations, plot_count)
      - capture:
         * Binary A: final binary_data (last element of all_intermediates)
         * Binary B: current_input (SHA256 digest) returned by function
         * Argon2: argon_hasher.hash(Binary B)  (or Binary A if you prefer)
    Returns lists indexed by plot_count-1: list_A, list_B, list_argon_bytes, plus metadata lengths.
    """
    binaries_A: List[bytes] = []
    binaries_B: List[bytes] = []
    argon_encoded_bytes: List[bytes] = []

    for p in range(1, max_plots + 1):
        if VERBOSE:
            print(f"[{time.strftime('%H:%M:%S')}] Running pipeline with plot_count = {p} ...")
            dbg(f"[{time.strftime('%H:%M:%S')}] Running pipeline with plot_count = {p} ...")

        # call your transformation function (must be defined/imported)
        # iterative_plot_transform returns: current_input (SHA256 digest), all_intermediates (list of binary_data), x_values_multi
        current_input, all_intermediates, xvals = iterative_plot_transform(password, salt1, salt2, iterations, plot_count=p)

        # Binary B = current_input (expected 32 bytes digest)
        binB = bytes(current_input) if isinstance(current_input, (bytes, bytearray)) else bytes(current_input)

        # Binary A = last binary_data from all_intermediates (raw float-packed bytes)
        if not all_intermediates:
            raise RuntimeError(f"iterative_plot_transform returned empty all_intermediates for plot_count={p}")
        binA = all_intermediates[-1]  # bytes

        # Argon2: hash the Binary B (you could choose Binary A instead if you want)
        # we store the encoded string bytes
        argon_encoded = argon_hasher.hash(binB)
        argon_bytes = argon_encoded.encode('utf-8')

        binaries_A.append(binA)
        binaries_B.append(binB)
        argon_encoded_bytes.append(argon_bytes)

    return binaries_A, binaries_B, argon_encoded_bytes

# All-pairs comparison and CSV write
def all_pairs_compare_and_save(binaries_A: List[bytes], binaries_B: List[bytes], argon_bytes: List[bytes], csv_path: str):
    """
    Compute all-pairs comparisons for each pair (i,j) with i<j:
      - binary A bit change count and percent
      - binary B bit change count and percent
      - argon encoded bytes bit change count and percent
    Writes CSV with columns:
      plot_i, plot_j,
      binA_bits_diff, binA_bits_total, binA_pct,
      binB_bits_diff, binB_bits_total, binB_pct,
      argon_bits_diff, argon_bits_total, argon_pct
    """
    n = len(binaries_A)
    header = [
        "plot_i", "plot_j",
        ...
        "binB_bits_diff", "binB_bits_total", "binB_pct",
        "argon_bits_diff", "argon_bits_total", "argon_pct",
        "binA_len_bytes", "binB_len_bytes", "argon_len_bytes"
    ]
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for i in range(n):
            for j in range(i+1, n):
                aA = binaries_A[i]; bA = binaries_A[j]
                aB = binaries_B[i]; bB = binaries_B[j]
                arga = argon_bytes[i]; argb = argon_bytes[j]

                 # Print debug info
                if VERBOSE:
                    dbg_bytes(f"Binary A (plot {i+1})", binaries_A[i])
                    dbg_bytes(f"Binary B (plot {i+1})", binaries_B[i])
                    dbg_bytes(f"Argon2 (plot {i+1})", argon_bytes[i])


                # Binary A bit metrics
                bitsA_total = max(len(aA), len(bA)) * 8
                bitsA_diff = bit_hamming(aA, bA)
                binA_pct = (bitsA_diff / bitsA_total * 100.0) if bitsA_total > 0 else 0.0

                # Binary B bit metrics
                bitsB_total = max(len(aB), len(bB)) * 8
                bitsB_diff = bit_hamming(aB, bB)
                binB_pct = (bitsB_diff / bitsB_total * 100.0) if bitsB_total > 0 else 0.0

                # Argon encoded bytes bit metrics
                bitsArg_total = max(len(arga), len(argb)) * 8
                bitsArg_diff = bit_hamming(arga, argb)
                arg_pct = (bitsArg_diff / bitsArg_total * 100.0) if bitsArg_total > 0 else 0.0

                writer.writerow([
                    (i+1), (j+1),
                    bitsA_diff, bitsA_total, f"{binA_pct:.6f}",
                    bitsB_diff, bitsB_total, f"{binB_pct:.6f}",
                    bitsArg_diff, bitsArg_total, f"{arg_pct:.6f}",
                    len(aA), len(aB), len(arga)
                ])

    print("All-pairs CSV written to:", csv_path)

# Quick summary print (optional) - prints an N x N percent matrix for each metric
def print_percent_matrix_from_csv(csv_path: str, metric_col_name: str):
    import pandas as pd
    df = pd.read_csv(csv_path)
    # Build matrix for metric (metric_col_name matches one of CSV columns)
    plots = sorted(set(df['plot_i'].tolist() + df['plot_j'].tolist()))
    n = max(plots)
    mat = np.full((n, n), np.nan)
    for _, row in df.iterrows():
        i = int(row['plot_i']) - 1
        j = int(row['plot_j']) - 1
        mat[i, j] = float(row[metric_col_name])
        mat[j, i] = mat[i, j]
    # diagonal zero
    for d in range(n):
        mat[d, d] = 0.0
    print(f"--- {metric_col_name} matrix (rows=plots i, cols=plots j) ---")
...
def print_results(results, password):
    ...
    dbg(f"\n=== Results for password: '{password}' ===")
    dbg(f"{'Method':<12} | {'Time (s)':<10} | {'Entropy':<10} | {'Output Len':<10} | {'Memory Usage':<25} | {'Verified':<8} | {'Sample Hash (truncated)'}")
    dbg("-"*110)
    for method, data in results.items():
        truncated_hash = (data['hash'][:40] + '...') if isinstance(data['hash'], str) and len(data['hash']) > 40 else (data['hash'] if isinstance(data['hash'], str) else data['hash'].hex()[:40] + '...')
        dbg(f"{method:<12} | {data['time']:<10.6f} | {data['entropy']:<10.4f} | {data['output_len']:<10} | {data['memory']:<25} | {str(data['verified']):<8} | {truncated_hash}")
        dbg(f"{'':<12}   Full Hash: {data['hash']}")
        dbg(f"{'':<12}   Salt     : {data['salt']}\n")    

if __name__ == "__main__":
    passwords = ["abc", "aba", "12345", "L:''[_=+)90*&^^%*/9YdD]"]
    methods = ["SHA256", "bcrypt", "scrypt", "Argon2", "Plot-only", "Plot+Argon2"]

     # produce binaries and argon bytes
    binaries_A, binaries_B, argon_bytes = produce_outputs_for_plot_counts(PASSWORD, SALT1, SALT2, ITERATIONS, MAX_PLOTS)

    # file for CSV results
    allpairs_csv = CSV_OUTPUT
    all_pairs_compare_and_save(binaries_A, binaries_B, argon_bytes, allpairs_csv)

    # optional: print summary matrices (binary-B percent, argon percent)
    # CSV columns: binA_pct column index 4 (zero-based header above), binB_pct index 7, argon_pct index 10.
    # but using function to read by column name is easier:
    print_percent_matrix_from_csv(allpairs_csv, 'binA_pct')   # raw binary_data matrix
    print_percent_matrix_from_csv(allpairs_csv, 'binB_pct')   # sha256-digest binary matrix
    print_percent_matrix_from_csv(allpairs_csv, 'argon_pct')  # argon2-encoded bytes matrix
