...
def seconds_to_human_readable(seconds: float) -> str:
    ...
    for unit_name, unit_sec in units:
        if seconds >= unit_sec:
            value = seconds / unit_sec
            return f"{value:.2f} {unit_name}"
    return f"{seconds:.2e} sec"


def estimate_cracking_times(entropy_bits: float, hash_time: float):
    total_guesses = 2 ** entropy_bits

    # Serial attacker → one CPU, must test half the search space
    serial_time_seconds = 0.5 * total_guesses * hash_time

    # Perfect attacker with infinite parallelism → one hash attempt
    parallel_time_seconds = hash_time

    return {
        "serial_seconds": serial_time_seconds,
        "serial_human": seconds_to_human_readable(serial_time_seconds),
        "parallel_seconds": parallel_time_seconds,
        "parallel_human": seconds_to_human_readable(parallel_time_seconds)
    }
...
if __name__ == "__main__":
    ...
    csv_filename = "password_benchmarks.csv"
    csv_headers = [
        "password", "method", "time_s", "entropy", "output_len", "memory_kb",
        "verified", "hash", "salt", "iterations",
        "crack_serial_sec", "crack_serial_human",
        "crack_parallel_sec", "crack_parallel_human"
    ]
    ...
    with open(csv_filename, mode="w", newline="", encoding="utf-8") as f_csv:
            ...
            for pwd in passwords:
                ...
                for method in methods:
                    ...
                    # NEW: compute cracking times automatically per method
                    crack = estimate_cracking_times(data["entropy"], data["time"])
                    ...
                    row = {
                        ...
                        "crack_serial_sec": crack["serial_seconds"],
                        "crack_serial_human": crack["serial_human"],
                        "crack_parallel_sec": crack["parallel_seconds"],
                        "crack_parallel_human": crack["parallel_human"]
                    }

                    writer.writerow(row)

    print(f"\n✅ All benchmark results with cracking times saved to '{csv_filename}'")